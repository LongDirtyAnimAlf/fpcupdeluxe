...
begin
  ...
  Processor:=TInstaller(Parent).Processor;
  Processor.Executable:=FRepoExecutable;
  Processor.Process.CurrentDirectory:=LocalRepository;
  ...
  begin
    ...
    FReturnOutput:=Processor.WorkerOutput.Text;
    //Processor.Environment.SetVar('GIT_SSL_NO_VERIFY',EnvStore);
  end
  else FReturnCode := 0;
  ...
  begin
    ...
    if Length(GetProxyCommand)>0 then
    begin
      ParamStore:=TStringList.Create;
      try
        ParamStore.Assign(Processor.Process.Parameters);
        Processor.Process.CommandLine:=DoubleQuoteIfNeeded(FRepoExecutable)+GetProxyCommand;
        FReturnCode:=Processor.ExecuteAndWait;
      finally
        Processor.Executable:=FRepoExecutable;
        Processor.Process.Parameters.Assign(ParamStore);
        ParamStore.Free;
      end;
    end;
...
procedure TGitClient.DisableSSL;
var
  Output:string;
begin
  if ExportOnly then exit;
  if NOT ValidClient then exit;
  if NOT DirectoryExists(LocalRepository) then exit;
  TInstaller(Parent).ExecuteCommandInDir(FRepoExecutable,['config','http.sslVerify','false'],LocalRepository, Output, '', Verbose);
end;
