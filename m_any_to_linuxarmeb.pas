...
{ Tany_linuxarmeb }
Tany_linuxarmeb = class(TCrossInstaller)
private
...
function Tany_linuxarmeb.GetLibs(Basepath:string): boolean;
const
  DirName='arm-linux';
//var
...
{$ifndef FPCONLY}
function Tany_linuxarmeb.GetLibsLCL(LCL_Platform: string; Basepath: string): boolean;
begin
...
function Tany_linuxarmeb.GetBinUtils(Basepath:string): boolean;
const
  DirName='arm-linux';
var
...
begin
  ...
  begin
    FBinsFound:=true;
    //if hardfloat then ShowInfo('Found hardfloat binary utilities. Please make sure you did NOT specified -dFPC_armeb in your FPCOPT !',etWarning);
...
procedure Tany_linuxarmeb.Reset;
begin
  inherited Reset;
...
var
  any_linuxarmeb:Tany_linuxarmeb;

initialization
  any_linuxarmeb:=Tany_linuxarmeb.Create;
  RegisterExtension(any_linuxarmeb.TargetCPU+'-'+any_linuxarmeb.TargetOS,any_linuxarmeb);
finalization
  any_linuxarmeb.Destroy;
end.
